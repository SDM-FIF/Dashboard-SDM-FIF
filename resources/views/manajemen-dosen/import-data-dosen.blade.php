    ...
    <main class="flex-1 p-4 md:p-6 min-h-screen">
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6 mb-6">
            <div class="flex items-center justify-between relative">
                ...
                {{-- Step 3: Selesai --}}
                <a href="{{ session()->has('import_result_dosen') ? route('manajemen-dosen.import.result') : 'javascript:void(0)' }}" 
                   class="flex flex-col items-center flex-1 relative {{ $currentStep == 3 ? 'text-green-600' : 'text-gray-400' }} {{ session()->has('import_result_dosen') ? 'cursor-pointer hover:opacity-80' : 'cursor-not-allowed' }}">
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-8">
            <div class="text-center">
                ...
                <h2 class="text-2xl font-bold text-[#C41E3A] mb-4">Template Import Data Dosen</h2>
                <p class="text-gray-600 mb-6">Klik tombol di bawah ini untuk mengunduh template Excel</p>
        ...
        <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6">
            ...
            {{-- Upload Form --}}
            <form action="{{ route('manajemen-dosen.import.upload') }}" method="POST" enctype="multipart/form-data" class="mb-6">
                @csrf
            ...
            <div>
                <h3 class="text-lg font-bold text-[#C41E3A] mb-2">Preview Import Data Dosen</h3>
                    ...
                    <table class="w-full border-collapse border border-gray-300 text-xs">
                        <thead>
                            <tr class="bg-[#C41E3A] text-white">
                                ...
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">NIP</th>
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">Kode Dosen</th>
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">Prodi</th>
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">Kelompok Keahlian</th>
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">JFA</th>
                                <th class="px-2 py-2 text-left text-xs font-semibold uppercase border border-gray-300">Status Pegawai</th>
                        ...
                        <tbody class="bg-white">
                            @if(session()->has('import_data_dosen') && count(session('import_data_dosen')) > 0)
                                ...
                                <tr class="{{ $row['is_valid'] ? 'hover:bg-gray-50' : 'bg-red-50' }} border-b border-gray-200">
                                    ...
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['nama_lengkap'] }}</td>
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['back_title'] ?? '-' }}</td>
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['nip'] }}</td>
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['kode_dosen'] }}</td>
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['prodi_name'] ?? '-' }}</td>
                                    <td class="px-2 py-2 text-xs border border-gray-300">{{ $row['kelompok_keahlian_name'] ?? '-' }}</td>
