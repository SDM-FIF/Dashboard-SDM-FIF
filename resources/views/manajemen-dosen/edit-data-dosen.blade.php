...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold text-[#C41E3A] tracking-tight">Edit Data Dosen</h1>
            <p class="text-sm text-gray-500 mt-1">Perbarui data personal, akademik, dan riwayat pendidikan dosen.</p>
        </div>

        {{-- Form Section --}}
        <form id="editDosenForm" action="{{ route('manajemen-dosen.update', $dosen->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 max-w-5xl">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="fakultas_id" 
                            ...
                            @if(isset($fakultas))
                                @foreach($fakultas as $fak)
                                    <option value="{{ $fak->id }}" {{ old('fakultas_id', $dosen->prodi->fakultas_id ?? '') == $fak->id ? 'selected' : '' }}>
                                        {{ $fak->nama_fakultas }}
                                    </option>
                                @endforeach
                            @endif
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="prodi_id" 
                            ...
                            @if(isset($prodi))
                                @foreach($prodi as $p)
                                    <option value="{{ $p->id }}" {{ old('prodi_id', $dosen->prodi_id) == $p->id ? 'selected' : '' }} class="prodi-option" data-fakultas="{{ $p->fakultas_id }}">
                                        {{ $p->nama_prodi }}
                                    </option>
                                @endforeach
                            @endif
                        ...
                        <select name="kelompok_keahlian_id" 
                            ...
                            @if(isset($kelompokKeahlian))
                                @foreach($kelompokKeahlian as $kk)
                                    <option value="{{ $kk->id }}" {{ old('kelompok_keahlian_id', $dosen->kelompok_keahlian_id) == $kk->id ? 'selected' : '' }}>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="date" name="tmt" value="{{ old('tmt', isset($dosen->tmt) ? \Carbon\Carbon::parse($dosen->tmt)->format('Y-m-d') : '') }}"
                               class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-[#F8FAFC] text-gray-700 text-sm focus:bg-white focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] transition-all outline-none">
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="status_dosen" 
                            ...
                            <option value="Tugas Belajar" {{ old('status_dosen', $dosen->status_dosen) == 'Tugas Belajar' ? 'selected' : '' }}>Tugas Belajar</option>
                            <option value="Izin Belajar" {{ old('status_dosen', $dosen->status_dosen) == 'Izin Belajar' ? 'selected' : '' }}>Izin Belajar</option>
                            <option value="CLTY" {{ old('status_dosen', $dosen->status_dosen) == 'CLTY' ? 'selected' : '' }}>Cuti (CLTY)</option>
                        </select>
                    </div>
                    ...
                    <div class="flex flex-col gap-1.5" id="tanggal_mulai_status_container" style="display: none;">
                        ...
                        <input type="date" name="tanggal_mulai_status" id="tanggal_mulai_status" value="{{ old('tanggal_mulai_status', isset($dosen->tanggal_mulai_status) ? \Carbon\Carbon::parse($dosen->tanggal_mulai_status)->format('Y-m-d') : '') }}"
                               class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-[#F8FAFC] text-gray-700 text-sm focus:bg-white focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] transition-all outline-none">
                    </div>
            ...
            @php
                $s1 = $dosen->riwayatPendidikan->where('jenjang', 'S1')->first();
                $s2 = $dosen->riwayatPendidikan->where('jenjang', 'S2')->first();
                $s3 = $dosen->riwayatPendidikan->where('jenjang', 'S3')->first();
            @endphp
                ...
                <div class="space-y-8">
                    ...
                    <div class="p-5 rounded-xl border border-gray-100 bg-[#F8FAFC] space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="riwayat[s1][nama_universitas]" value="{{ old('riwayat.s1.nama_universitas', $s1->nama_universitas ?? '') }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="riwayat[s1][prodi_pendidikan]" value="{{ old('riwayat.s1.prodi_pendidikan', $s1->prodi_pendidikan ?? '') }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="date" name="riwayat[s1][tanggal_lulus]" value="{{ old('riwayat.s1.tanggal_lulus', isset($s1->tanggal_lulus) ? \Carbon\Carbon::parse($s1->tanggal_lulus)->format('Y-m-d') : '') }}" required class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Ijazah (PDF/JPG/PNG)</label>
                                ...
                                @if(isset($s1->ijazah))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s1->ijazah }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-pdf"></i> Lihat Ijazah S1 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Transkrip Nilai (PDF/JPG/PNG)</label>
                                ...
                                @if(isset($s1->transkrip_nilai))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s1->transkrip_nilai }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-alt"></i> Lihat Transkrip S1 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="p-5 rounded-xl border border-gray-100 bg-[#F8FAFC] space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="flex flex-col gap-1.5">
                                ...
                                <input type="text" name="riwayat[s2][nama_universitas]" value="{{ old('riwayat.s2.nama_universitas', $s2->nama_universitas ?? '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Program Studi</label>
                                <input type="text" name="riwayat[s2][prodi_pendidikan]" value="{{ old('riwayat.s2.prodi_pendidikan', $s2->prodi_pendidikan ?? '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Tanggal Lulus</label>
                                <input type="date" name="riwayat[s2][tanggal_lulus]" value="{{ old('riwayat.s2.tanggal_lulus', isset($s2->tanggal_lulus) ? \Carbon\Carbon::parse($s2->tanggal_lulus)->format('Y-m-d') : '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Ijazah (PDF/JPG/PNG)</label>
                                ...
                                @if(isset($s2->ijazah))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s2->ijazah }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-pdf"></i> Lihat Ijazah S2 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
                            <div class="flex flex-col gap-1.5">
                                ...
                                @if(isset($s2->transkrip_nilai))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s2->transkrip_nilai }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-alt"></i> Lihat Transkrip S2 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="p-5 rounded-xl border border-gray-100 bg-[#F8FAFC] space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama Universitas</label>
                                <input type="text" name="riwayat[s3][nama_universitas]" value="{{ old('riwayat.s3.nama_universitas', $s3->nama_universitas ?? '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Program Studi</label>
                                <input type="text" name="riwayat[s3][prodi_pendidikan]" value="{{ old('riwayat.s3.prodi_pendidikan', $s3->prodi_pendidikan ?? '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Tanggal Lulus</label>
                                <input type="date" name="riwayat[s3][tanggal_lulus]" value="{{ old('riwayat.s3.tanggal_lulus', isset($s3->tanggal_lulus) ? \Carbon\Carbon::parse($s3->tanggal_lulus)->format('Y-m-d') : '') }}" class="w-full px-4 py-3 border border-gray-200 rounded-xl bg-white text-gray-700 text-sm focus:ring-2 focus:ring-red-200 focus:border-[#C41E3A] outline-none transition-all">
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Ijazah (PDF/JPG/PNG)</label>
                                <input type="file" name="riwayat[s3][ijazah]" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-3 py-2 border border-gray-200 rounded-xl bg-white text-xs text-gray-600 focus:ring-2 focus:ring-red-200 outline-none">
                                @if(isset($s3->ijazah))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s3->ijazah }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-pdf"></i> Lihat Ijazah S3 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-xs font-semibold text-gray-500 uppercase tracking-wider">Ganti Transkrip Nilai (PDF/JPG/PNG)</label>
                                <input type="file" name="riwayat[s3][transkrip_nilai]" accept=".pdf,.jpg,.jpeg,.png" class="w-full px-3 py-2 border border-gray-200 rounded-xl bg-white text-xs text-gray-600 focus:ring-2 focus:ring-red-200 outline-none">
                                @if(isset($s3->transkrip_nilai))
                                    <div class="mt-1">
                                        <a href="/storage/{{ $s3->transkrip_nilai }}" target="_blank" class="text-xs text-blue-600 hover:underline inline-flex items-center gap-1">
                                            <i class="fas fa-file-alt"></i> Lihat Transkrip S3 Saat Ini
                                        </a>
                                    </div>
                                @endif
                            </div>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4 pt-6 border-t border-gray-200 max-w-5xl">
                ...
                <div class="flex items-center gap-3 w-full sm:w-auto">
                    ...
                    <button type="submit" 
                            class="flex-1 sm:flex-none px-6 py-3 bg-[#FBB03B] hover:bg-[#E09A2A] text-black font-semibold rounded-xl text-sm transition-all shadow-sm font-bold">
                        Perbarui Data Dosen
                    </button>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (fakultasSelect && prodiSelect) {
                ...
                // Trigger change on load if value exists to align
                if(fakultasSelect.value) {
                    fakultasSelect.dispatchEvent(new Event('change'));
                    // Set value again to ensure it remains correct
                    prodiSelect.value = "{{ old('prodi_id', $dosen->prodi_id) }}";
                }
            }

            // Form Submit handler via AJAX for beautiful feedback & updating riwayat
            ...
            if (form) {
                form.addEventListener('submit', function(e) {
                    ...
                    // Send AJAX Fetch (using method spoofing PUT)
                    const formData = new FormData(form);
                    formData.append('_method', 'PUT');

                    fetch(form.action, {
                        method: 'POST', // must send as POST with _method PUT when uploading files
                        headers: {
                    ...
                    .then(data => {
                        if (data.success) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
                                text: data.message || 'Data dosen berhasil diperbarui',
                            ...
                            let errorMessage = 'Gagal memperbarui data dosen';
                            if (data.message) {
                    ...
                    .catch(error => {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error Sistem',
                            text: 'Terjadi kesalahan sistem saat memperbarui data: ' + error.message,
                            confirmButtonColor: '#C41E3A',
                            customClass: {
                                popup: 'rounded-2xl',
                                confirmButton: 'px-5 py-2.5 rounded-xl font-semibold text-sm'
