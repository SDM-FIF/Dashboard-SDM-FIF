...
<head>
    ...
    <title>Tambah Kelompok Keahlian - Dashboard SDM FIF</title>
...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        ...
        <div class="mb-8 mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl md:text-4xl font-extrabold text-[#C41E3A] tracking-tight">Tambah Kelompok Keahlian</h1>
                <p class="text-sm text-gray-500 mt-1 font-medium">Buat data kelompok keahlian baru untuk klasifikasi dosen.</p>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 max-w-3xl hover:shadow-md transition-shadow duration-300">
            <form action="{{ route('kelompok-keahlian.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="nama_kelompok_keahlian" id="nama_kelompok_keahlian" value="{{ old('nama_kelompok_keahlian') }}" required
