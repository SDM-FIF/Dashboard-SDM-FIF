...
<head>
    ...
    <title>Edit Data TPA - Dashboard SDM FIF</title>
...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        ...
        <div class="mb-8 mt-4">
            ...
            <h1 class="text-3xl md:text-4xl font-extrabold text-[#C41E3A] tracking-tight">Edit Data TPA</h1>
            <p class="text-sm text-gray-500 mt-1">Ubah informasi data Tenaga Kependidikan dan Profesional.</p>
        </div>

        {{-- Form Section --}}
        <form action="{{ route('manajemen-tpa.update', $tpa->id) }}" method="POST" class="space-y-8 max-w-5xl">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-md transition-shadow duration-300 hidden">
                <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-50">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500">Ubah password atau username akun login TPA</p>
                    </div>
                </div>
