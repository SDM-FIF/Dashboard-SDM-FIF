...
<head>
    ...
    <title>Tambah Data Mahasiswa - Dashboard SDM FIF</title>
...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        ...
        <div class="mb-8 mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-3xl md:text-4xl font-extrabold text-[#C41E3A] tracking-tight">Tambah Data Mahasiswa</h1>
                <p class="text-sm text-gray-500 mt-1 font-medium">Tambah entri baru data mahasiswa ke dalam sistem database.</p>
            </div>
            ...
            <form action="{{ route('mahasiswa.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="number" name="nim" value="{{ old('nim') }}" required placeholder="Masukkan nomor induk mahasiswa" 
                    ...
                    {{-- Status --}}
                    <div class="flex flex-col gap-1.5">
                        <label class="text-xs font-bold text-gray-400 uppercase tracking-wider">Status <span class="text-red-500">*</span></label>
                        <select name="status" required 
                            ...
                            @foreach($statuses as $status)
                                <option value="{{ $status }}" {{ old('status') == $status ? 'selected' : ($status == 'aktif' ? 'selected' : '') }}>{{ ucfirst($status) }}</option>
                            @endforeach
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
    ...
    @if($errors->has('error'))
        <div class="fixed bottom-4 right-4 bg-red-600 text-white px-6 py-4 rounded-xl shadow-2xl z-50 animate-pulse">
