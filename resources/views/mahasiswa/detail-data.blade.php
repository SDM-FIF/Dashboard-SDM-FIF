                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            @php
                                // Gunakan strtolower agar pengecekan aman meski di DB kecil semua
                                ...
                                $statusClass = match ($statusCurrent) {
                                    ...
                                    'nonaktif', 'resign', 'dikeluarkan' => 'bg-red-100 text-red-800',
                        ...
                        <div>
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('mahasiswa.edit', $mahasiswa->id) }}"
                                    class="text-blue-600 hover:underline text-sm font-medium">Edit Data</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <form method="GET" action="{{ route('mahasiswa.show', $mahasiswa->id) }}" class="mb-6">
                    <div class="flex flex-wrap items-center gap-4">
                        <select name="filter_status"
                            class="px-4 py-2 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 outline-none">
                        ...
                        <button type="submit"
                            class="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-lg font-semibold transition-all shadow-md">
                            <i class="fas fa-filter mr-2"></i>Filter
                        </button>
                        <a href="{{ route('mahasiswa.show', $mahasiswa->id) }}"
                            class="text-gray-500 hover:text-gray-700 underline text-sm">Reset</a>
                    </div>
                </form>
            </div>
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($allMahasiswa as $index => $item)
                            <tr
                                class="{{ $item->id == $mahasiswa->id ? 'bg-blue-50 border-l-4 border-blue-500' : 'hover:bg-gray-50' }}">
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex space-x-3">
                                        <a href="{{ route('mahasiswa.show', $item->id) }}"
                                            class="text-blue-600 hover:text-blue-800" title="Detail"><i
                                                class="fas fa-eye"></i></a>
                                        <a href="{{ route('mahasiswa.edit', $item->id) }}"
                                            class="text-green-600 hover:text-green-800" title="Edit"><i
