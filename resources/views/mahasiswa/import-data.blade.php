...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        ...
        <div class="mb-8">
            ...
            <p class="text-sm text-gray-500 mt-1">Import data mahasiswa secara masal menggunakan spreadsheet Excel/CSV.</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center max-w-2xl mx-auto hover:shadow-md transition-shadow">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Template Import Data Mahasiswa</h2>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-md transition-shadow">
            ...
            <div class="max-w-2xl mx-auto mb-8 text-center">
                ...
                <form action="{{ route('mahasiswa.import.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
            ...
            <div class="pt-6 border-t border-gray-50">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Preview Data Mahasiswa</h3>
                    ...
                    <table class="min-w-full text-xs">
                        <thead>
                            <tr class="bg-[#F8FAFC] text-gray-500 border-b border-gray-100">
                                ...
                                <th class="px-3 py-3 text-left font-bold uppercase">Program Studi</th>
                            ...
                            @if(session()->has('import_data') && count(session('import_data')) > 0)
                                ...
                                <tr class="{{ $row['is_valid'] ? 'bg-white hover:bg-gray-50' : 'bg-red-50/40 hover:bg-red-50/60' }} transition-colors">
                                    ...
                                    <td class="px-3 py-3.5">
                                        @php
                                            $badgeColor = match(strtoupper($row['status'])) {
                                                'AKTIF' => 'bg-emerald-50 text-emerald-700 border-emerald-100',
                                                'CUTI' => 'bg-amber-50 text-amber-700 border-amber-100',
                                                'TIDAK AKTIF', 'NONAKTIF' => 'bg-rose-50 text-rose-700 border-rose-100',
                                                default => 'bg-gray-50 text-gray-700 border-gray-100'
                                            };
                                        @endphp
                                        <span class="inline-flex items-center px-2 py-0.5 rounded border font-bold text-[10px] {{ $badgeColor }}">
                                            {{ $row['status'] }}
                                        </span>
                                    </td>
                                    <td class="px-3 py-3.5">
                ...
                <div class="mt-6 flex items-center justify-between gap-4 border-t border-gray-100 pt-6">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @if($validCount > 0)
                        <form action="{{ route('mahasiswa.import.save') }}" method="POST">
                            @csrf
                            <button type="submit" 
