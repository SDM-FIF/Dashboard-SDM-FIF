...
<head>
    ...
    <title>Import Mahasiswa Kompetisi - Dashboard SDM FIF</title>
...
<body class="flex flex-col md:flex-row bg-[#F8FAFC] min-h-screen text-[#1E293B]">
    ...
    <main class="flex-1 p-6 md:p-8 overflow-y-auto">
        {{-- Header Section --}}
        <div class="mb-8">
            <h1 class="text-3xl md:text-4xl font-extrabold text-[#C41E3A] tracking-tight">Import Mahasiswa Kompetisi</h1>
            <p class="text-sm text-gray-500 mt-1">Import partisipasi kompetisi mahasiswa secara masal menggunakan spreadsheet Excel/CSV.</p>
        </div>
        ...
        @php
            if(request()->has('step')) {
                ...
                $currentStep = 1;
                if(session()->has('import_kompetisi_data')) {
                    $currentStep = 2;
                }
            ...
            $fileUploaded = session()->has('kompetisi_file_uploaded');
        @endphp
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8 hover:shadow-md transition-shadow">
            <div class="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-0">
                ...
                {{-- Step 1 --}}
                <a href="{{ route('kompetisi.import.view', ['step' => 1]) }}" 
                ...
                {{-- Step 2 --}}
                <a href="{{ route('kompetisi.import.view', ['step' => 2]) }}" 
                   class="flex flex-col items-center text-center md:flex-1 group">
                    <div class="w-12 h-12 rounded-full flex items-center justify-center font-bold text-sm mb-2 transition-all border-2 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 text-center max-w-2xl mx-auto hover:shadow-md transition-shadow">
            ...
            <h2 class="text-2xl font-bold text-gray-800 mb-2">Template Import Mahasiswa Kompetisi</h2>
            ...
            <a href="{{ route('kompetisi.import.download.template') }}" 
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8 hover:shadow-md transition-shadow">
            ...
            <div class="max-w-2xl mx-auto mb-8 text-center">
                ...
                <form action="{{ route('kompetisi.import.upload') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
            ...
            <div class="pt-6 border-t border-gray-50">
                <div class="flex items-center justify-between mb-4 flex-wrap gap-2">
                    <div>
                        <h3 class="text-lg font-bold text-gray-800">Preview Data Kompetisi</h3>
                        <p class="text-xs text-gray-500 mt-0.5">Sistem memvalidasi format data di bawah ini secara real-time</p>
                    </div>
                    @if(session()->has('import_kompetisi_data') && count(session('import_kompetisi_data')) > 0)
                        @php
                            $validCount = collect(session('import_kompetisi_data'))->where('is_valid', true)->count();
                            $invalidCount = collect(session('import_kompetisi_data'))->where('is_valid', false)->count();
                        @endphp
                    ...
                    <table class="min-w-full text-xs">
                        <thead>
                            <tr class="bg-[#F8FAFC] text-gray-500 border-b border-gray-100">
                                ...
                                <th class="px-3 py-3 text-left font-bold uppercase">NIM</th>
                                <th class="px-3 py-3 text-left font-bold uppercase">Nama Kompetisi</th>
                                <th class="px-3 py-3 text-left font-bold uppercase">Juara</th>
                                <th class="px-3 py-3 text-left font-bold uppercase">Kesalahan / Error</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @if(session()->has('import_kompetisi_data') && count(session('import_kompetisi_data')) > 0)
                                @foreach(session('import_kompetisi_data') as $row)
                                <tr class="{{ $row['is_valid'] ? 'bg-white hover:bg-gray-50' : 'bg-red-50/40 hover:bg-red-50/60' }} transition-colors">
                                    ...
                                    <td class="px-3 py-3.5 font-semibold text-gray-500 font-mono">{{ $row['nim'] }}</td>
                                    <td class="px-3 py-3.5 font-bold text-gray-800">{{ $row['nama_kompetisi'] }}</td>
                                    <td class="px-3 py-3.5 text-gray-600">{{ $row['juara'] ?? '-' }}</td>
                ...
                <div class="mt-6 flex items-center justify-between gap-4 border-t border-gray-100 pt-6">
                    ...
                    <div class="flex items-center gap-3">
                        <a href="{{ route('kompetisi.import.view', ['step' => 2, 'reset' => 1]) }}" 
                        ...
                        @if($validCount > 0)
                        <form action="{{ route('kompetisi.import.save') }}" method="POST">
                            @csrf
                            <button type="submit" 
